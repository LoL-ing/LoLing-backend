...
class Match_histories(BaseIdModel):
    ...
    level = Column(Integer, nullable=False, comment="해당 match 내에서 user의 level을 나타낸다.")
    CS = Column(
        Integer,
        nullable=False,
    ...
    gold = Column(Integer, nullable=False, comment="해당 match에서 user가 획득한 총 gold 양이다.")
    play_duration = Column(Text, nullable=False, comment="match의 play 총 시간이다. ")
    play_time = Column(Text, nullable=False, comment="match가 발생한 날짜 정보이다.")
    queue_type = Column(
        Integer, nullable=False, comment="해당 match의 type(자유 랭크인지 솔로 랭크인지) 에 대한 고유 번호이다."
    )
    summoner_name = Column(
        String(20), nullable=False, comment="각 match의 참여한 개별 소환사의 이름을 나타낸다."
    )
    match_id = Column(
        String(10), nullable=False, comment="라이엇 API 가 제공하는 해당 match 의 고유 번호이다."
    )
    line_name = Column(
        String(20), nullable=False, comment="해당 match에서 user가 플레이한 line 이름이다. "
    )
    champion_name_en = Column(
        String(20), nullable=False, comment="해당 match에서 user가 플레이한 champion 이름이다."
    )
    kill = Column(Integer, nullable=False, comment="해당 match에서 user가 kill한 횟수이다.")
    death = Column(Integer, nullable=False, comment="해당 match에서 user가 death한 횟수이다.")
    assist = Column(Integer, nullable=False, comment="해당 mathc에서 user가 assist한 횟수이다.")
    win_or_lose = Column(
        Integer, nullable=False, comment="해당 match에서 user가 이겼는지 졌는지를 나타내는 요소이다."
    )
...
class Current_season_summaries(BaseIdModel):
    ...
    queue_id = Column(
        Integer, nullable=False, comment="해당 Season Summary가 속한 queue 유형의 고유 번호이다."
    )
    rank = Column(
        Integer,
        nullable=False,
    ...
    tier_id = Column(
        Integer, nullable=False, comment="해당 Season Summary가 속한 tier의 고유 번호이다."
    )
    wins = Column(
        Integer, nullable=False, comment="해당 current season summary의 시즌 전체 승리 횟수이다."
    )
