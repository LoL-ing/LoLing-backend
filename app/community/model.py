...
class Attachments(BaseIdModel):
    ...
    image_url = Column(
        String(2083), nullable=False, comment="attachment의 주소. 사진을 불러오기 위한 필수 요소이다."
    )
    order = Column(
        Integer,
        nullable=False,
...
class Boards(BaseIdModel):
    ...
    __table_args__ = {"schema": "COMMUNITY"}
    title = Column(String(10), nullable=False, unique=True, comment="board의 제목이다.")
...
class Comments(BaseIdModel):
    ...
    parent_post_id = Column(
        Integer, nullable=False, comment="comment가 달린 게시글의 고유 번호이다. "
    )
    user_id = Column(
        Integer,
        nullable=False,
...
class Posts(BaseIdModel):
    ...
    user_id = Column(
        Integer, nullable=False, comment="게시물을 작성한 user의 id. 누가 작성하였는지 구분하기 위한 필수 요소이다."
    )
...
class Reported_comments(BaseIdModel):
    ...
    __table_args__ = {"schema": "COMMUNITY"}
    description = Column(
        String(300), nullable=False, comment="신고된 comment 에 대한 신고 상세 설명이다."
    )
    reported_comment_id = Column(
        Integer, nullable=False, comment="신고당한 comment의 고유 번호(Comments)이다. "
    )
    reporter_user_id = Column(
        Integer, nullable=False, comment="신고한 유저의 고유 번호(Users)이다. "
    )
...
class Reported_posts(BaseIdModel):
    ...
    __table_args__ = {"schema": "COMMUNITY"}
    description = Column(String(300), nullable=False, comment="post 신고에 대한 신고 상세 설명이다.")
    reported_post_id = Column(
        Integer, nullable=False, comment="신고당한 post의 고유 번호(Posts)이다. "
    )
    reporter_user_id = Column(
        Integer, nullable=False, comment="신고한 유저의 고유 번호(Users)이다. "
    )
...
class Reported_Users(BaseIdModel):
    ...
    reported_user_id = Column(
        Integer, nullable=False, comment="신고당한 유저의 고유 번호(Users)이다."
    )
    reporter_user_id = Column(
        Integer, nullable=False, comment="신고한 유저의 고유 번호(Users)이다. "
    )
    description = Column(String(300), nullable=False, comment="user 신고에 대한 신고 상세 설명이다.")
